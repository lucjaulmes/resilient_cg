...
void force_checkpoint(checkpoint_data *ckpt_data, double *iterate, double *gradient, double *p, double *Ap)
{
	...
	double *old_rho = mp.old_rho, *alpha = mp.alpha;

	#pragma omp task out(*behaviour) inout(*ckpt_data, *alpha, *old_rho) label(force_checkpoint) no_copy_deps
	{
		*ckpt_data->save_rho = *old_rho;
...
void due_checkpoint(checkpoint_data *ckpt_data, double *iterate, double *gradient, double *p, double *Ap)
{
	...
	// reuse pragma check
	#pragma omp task out(*behaviour) inout(*ckpt_data, *old_rho) label(due_checkpoint) no_copy_deps
	{
		if(!aggregate_skips())
		{
...
void force_rollback(checkpoint_data *ckpt_data, double *iterate, double *gradient, double *p, double *Ap)
{
	...
	#pragma omp task out(*behaviour) inout(*ckpt_data, *alpha, *old_rho) label(force_rollback) no_copy_deps
	{
		*old_rho = *ckpt_data->save_rho;
		*alpha = *ckpt_data->save_alpha;
		*behaviour = RELOAD_CHECKPOINT;
