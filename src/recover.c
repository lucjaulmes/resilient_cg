...
void recover_inverse(const Matrix *A, Precond *M, const double *b, const double *g, double *x, int *lost_blocks, const int nb_lost)
{
	...
	{
		if(recovery_sizes[i] == 1 && get_failblock_size() == M->N[0]->L->n)
			do_free_interpolation(A, b, g, x, M, lost_blocks[pos]);
		else
...
void do_free_interpolation(const Matrix *A, const double *b, const double *g, double *x, Precond *M, const int lost_block)
{
	...
	int fbs = get_failblock_size(), lost = lost_block << log2fbs;
	...
	cs_lsolve(M->N[lost_block]->L, inv);					// inv = L\inv
	cs_ltsolve(M->N[lost_block]->L, inv);					// inv = L'\inv
	cs_pvec(fbs, M->S[lost_block]->Pinv, inv, &x[lost]);	// x   = P'*inv
...
int recover_p_repeat(magic_pointers *mp, double *p, const double *old_p, int block)
{
	...
	if(!is_skipped_block(block, 1 << get_data_vectptr(old_p)) && !is_skipped_block(block, MASK_GRADIENT))
	{
		...
		daxpy(fbs, *(mp->beta), old_p + blockpos, z + blockpos, p + blockpos);
...
int recover_z(magic_pointers *mp, double *z, int block)
{
	...
	if(!is_skipped_block(block, MASK_GRADIENT))
	{
		int fbs = get_failblock_size(), blockpos = block << get_log2_failblock_size();
		if(blockpos + fbs > mp->A->n)
			fbs = mp->A->n - blockpos;
		...
		cs_ipvec(fbs, mp->M->S[block]->Pinv, &mp->g[blockpos], x);	// x = P*g
		cs_lsolve(mp->M->N[block]->L, x) ;							// x = L\x
		cs_ltsolve(mp->M->N[block]->L, x) ;							// x = L'\x
		cs_pvec(fbs, mp->M->S[block]->Pinv, x, &z[blockpos]);		// z = P'*x
	...
	log_err(SHOW_FAILINFO, "\tRepeating preconditioning for block %d of z %d\n", block, r);
...
int recover_x_lossy(magic_pointers *mp, double *x)
{
	...
	recover_inverse(mp->A, mp->M, mp->b, NULL, x, lost_blocks, nb_lost);
...
int recover_full_xk(magic_pointers *mp, double *x, const int mark_clean)
{
	...
	if(!overlapping_faults(MASK_GRADIENT, MASK_ITERATE))
	{
		...
		recover_inverse(mp->A, mp->M, mp->b, g, x, lost_blocks, nb_lost);
...
int recover_full_p_invert(magic_pointers *mp, double *p, const int mark_clean)
{
	...
	if(!overlapping_faults(MASK_A_P, mask))
	{
		...
		recover_inverse(mp->A, mp->M, Ap, NULL, p, lost_blocks, nb_lost);
...
int recover_full_old_p_invert(magic_pointers *mp, double *old_p, const int mark_clean)
{
	...
	if(r == 0)
	{
		recover_inverse(mp->A, mp->M, old_Ap, NULL, old_p, lost_blocks, nb_lost);
...
int recover_only_fails_Ap(magic_pointers *mp, double *Ap, const double *p, const int mark_clean)
{
	...
	for(i=0; i < get_nb_failblocks(); i++)
	{
		if(! is_failed_not_skipped_block(i, MASK_A_P))
			continue;
...
int recover_full_z(magic_pointers *mp, double *z, const int mark_clean)
{
	...
	for(i=0; i < get_nb_failblocks(); i++)
	{
		if(! is_skipped_block(i, MASK_Z))
			continue;

		rr = recover_z(mp, z, i);

		if(mark_clean && !rr)
			mark_corrected(i, MASK_Z);
