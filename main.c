...
int MAGIC_MAXITERATION = 0;
int MAGIC_NBSIMULTANEOUSFAULTS = 1;
...
int read_param(int argsleft, char* argv[], double *lambda, int *restart, int *runs, int *fail_size)
{
	...
	else if( strcmp(argv[0], "-maxit") == 0 )
	{
		...
		MAGIC_MAXITERATION = strtod(argv[1], NULL);

		if( MAGIC_MAXITERATION < 0 )
			usage(argv[0]);
	...
	else if( strcmp(argv[0], "-nsf") == 0 )
	{
		// we want at least the double and a matrix market file after
		if( argsleft <= 2 )
			usage(argv[0]);

		MAGIC_NBSIMULTANEOUSFAULTS = strtod(argv[1], NULL);

		if( MAGIC_NBSIMULTANEOUSFAULTS < 0 )
			usage(argv[0]);
...
int main(int argc, char* argv[])
{
	...
	for(f=1; f<argc; f += nb_read )
		...
		if( (nb_read = read_param(argc - f, &argv[f], &lambda, &restart, &runs, &fail_size)) == 0 )
		{
			...
			printf("lambda:%e strategy:%s failure_size=%dB\n", lambda, strat, fail_size);
			...
			for(j=0;j<runs;j++)
			{
				...
				setup(&matrix, fail_size, lambda, 0.7);

				// if symmetric, solve with conjugate gradient method
				if(symmetric)
					solve_pcg(n, &matrix, b, x, 1e-10 );
				//otherwise, gmres
				else
					fprintf(stderr, "Oops, PCG only solves symmetric matrices ! \n");
