...
{
	if( n == SINGLEFAULT )
		strcpy(name, "single fault");	
	else if( n == MULTFAULTS_GLOBAL )
		strcpy(name, "multiple faults, global recovery");	
	else if( n == MULTFAULTS_UNCORRELATED )
		strcpy(name, "multiple faults, uncorrelated recovery");	
	else if( n == MULTFAULTS_DECORRELATED )
		strcpy(name, "multiple faults, decorrelated recovery");	
	else
		strcpy(name, "unknown fault strategy, expect crashes");	
}
    ...
    for(f=1; f<argc; f++)
		...
		else
		{
			...
			if(mm_is_sparse(matcode))
			{
				...
				printf("using SPARSE matrix format ; ");
			}
			else
			{
				matrix = &dA;
				...
				printf("using DENSE matrix format ; ");
			}

			if(symmetric)
				printf("symmetric matrix : Conjugate Gradient\n");
			else
			{
				printf("non-symmetric matrix : ");
				if( restart > 0 )
					printf("restarted GMRES(%d)\n", restart);
				else
					printf("full GMRES\n");
			}
			...
			printf("Parameters : lambda = %e (~1/MTBF in usec) and block size = %d, %s\n", lambda, block_size, strat);

			// now generate random rhs to problem, and initialize first guess to 0
