...
void submatrix_sparse_nodiagblocks( const SparseMatrix *A , const int *rows, const int nr, const int *cols, const int nc, const int bs, SparseMatrix *B )
{
	// nb = Number of Blocks, bs = Block Size
	int i, ii, j, jj, k, p = 0;
	...
	if( rows[nr-1] + bs > A->n )
		B->n -= rows[nr-1] + bs - A->n;

	if( cols[nc-1] + bs > A->m )
		B->m -= cols[nc-1] + bs - A->m;

	B->nnz = 0;
	// first, count the nnz's (no easy estimation is reasonable)
	for(i=0, k=0; i<nr; i++)
		for(ii=rows[i]; ii < rows[i] + bs && ii < A->n && k < B->n ; ii++, k++)
			for(j=A->r[ii], jj = 0; j < A->r[ii+1]; j++)
			{
	...
	allocate_sparse_matrix(B->n, B->m, B->nnz, B);
	...
	for(i=0, k=0; i<nr; i++)
		for(ii=rows[i]; ii < rows[i] + bs && ii < A->n && k < B->n ; ii++, k++)
		{
			...
			for(j=A->r[ii], jj = 0; j < A->r[ii+1]; j++)
			{
				...
				// remove diagonal-block elements
				if( (ii / bs) == (A->c[j] / bs) )
					continue;
